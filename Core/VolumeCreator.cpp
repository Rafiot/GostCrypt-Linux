...
namespace GostCrypt
{
	...
	void VolumeCreator::CreationThread ()
	{
		try
		{
			...
			{
				if (filesystemSize < GST_MIN_FAT_FS_SIZE || filesystemSize > GST_MAX_FAT_SECTOR_COUNT * Options->SectorSize)
					throw ParameterIncorrect (SRC_POS);
	...
	void VolumeCreator::CreateVolume (shared_ptr <VolumeCreationOptions> options)
	{
		...
		try
		{
			...
			if (options->Path.IsDevice())
			{
				...
				if (options->SectorSize < GST_MIN_VOLUME_SECTOR_SIZE
					|| options->SectorSize > GST_MAX_VOLUME_SECTOR_SIZE
#if !defined (GST_LINUX)
					|| options->SectorSize != GST_SECTOR_SIZE_LEGACY
#endif
