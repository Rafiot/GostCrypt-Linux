...
#ifdef GST_WINDOWS
		// Hotkeys
		GST_CHECK_BOX_VALIDATOR (BeepAfterHotkeyMountDismount);
		GST_CHECK_BOX_VALIDATOR (DisplayMessageAfterHotkeyDismount);
#endif

		TransferDataToWindow();		// Code below relies on TransferDataToWindow() called at this point

#if defined (GST_WINDOWS) || defined (GST_MACOSX)
