		...
		case Step::VolumeSize:
			{
				...
				else if (SelectedVolumeType == VolumeType::Hidden)
				{
					...
					pageText = LangString["SIZE_HELP_HIDDEN_VOL"] + L"\n\n" + _("Please note that if your operating system does not allocate files from the beginning of the free space, the maximum possible hidden volume size may be much smaller than the size of the free space on the outer volume. This not a bug in GostCrypt but a limitation of the operating system.");
				...
				if (OuterVolume)
					page->SetMinVolumeSize (GST_MIN_HIDDEN_VOLUME_HOST_SIZE);
				else if (SelectedVolumeType == VolumeType::Hidden)
					page->SetMinVolumeSize (GST_MIN_HIDDEN_VOLUME_SIZE);
				else
					page->SetMinVolumeSize (GST_MIN_VOLUME_SIZE);
		...
		case Step::VolumeLocation:
			{
				...
				if (forward)
				{
					...
					if (SelectedVolumePath.IsDevice())
					{
						...
						SectorSize = GST_SECTOR_SIZE_FILE_HOSTED_VOLUME;
				}
		...
		case Step::VolumePassword:
			{
				...
				{
					if (VolumeSize <= GST_MAX_FAT_SECTOR_COUNT * SectorSize)
		...
		case Step::CreationProgress:
			{
				...
				if (forward)
				{
					...
					{
						if (OuterVolume && VolumeSize > GST_MAX_FAT_SECTOR_COUNT * SectorSize)
						{
							uint64 limit = GST_MAX_FAT_SECTOR_COUNT * SectorSize / BYTES_PER_TB;
