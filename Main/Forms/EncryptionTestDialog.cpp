		...
		try
		{
			...
			if (xts)
			{
				...
				try
				{
					...
					blockNumberLim = (ea->GetMaxBlockSize() == 16 ? BLOCKS_PER_XTS_DATA_UNIT : BLOCKS_PER_XTS_DATA_UNIT_SMALL) - 1;
					if (blockNumber > blockNumberLim)
					{
						blockNumber = blockNumberLim;
						if (blockNumberLim == 31)
							BlockNumberTextCtrl->SetValue (L"31");
						else
							BlockNumberTextCtrl->SetValue (L"63");
					}
				}
				catch (...)
				{
