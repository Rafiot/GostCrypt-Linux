...
namespace GostCrypt
{
	...
	bool GraphicUserInterface::OnInit ()
	{
		...
		try
		{
			...
			if (SingleInstanceChecker->IsAnotherRunning())
			{
				...
				Gui->ShowInfo (_("GostCrypt is already running."));
				return false;
			}
	...
	{
		wxString url = wxString (StringConverter::ToWide (secure ? GST_APPLINK_SECURE : GST_APPLINK)) + L"&dest=" + linkId;
...
#ifdef GST_RESOURCE_DIR
			docPath = StringConverter::ToWide (string (GST_TO_STRING (GST_RESOURCE_DIR)) + "/doc/GostCrypt User Guide.pdf");
#elif defined (GST_WINDOWS)
			docPath += L"\\GostCrypt User Guide.pdf";
#elif defined (GST_MACOSX)
			docPath += L"/../Resources/GostCrypt User Guide.pdf";
#elif defined (GST_UNIX)
			docPath = L"/usr/share/gostcrypt/doc/GostCrypt User Guide.pdf";
#else
#	error GST_RESOURCE_DIR undefined
#endif
	...
	FilePath GraphicUserInterface::SelectVolumeFile (wxWindow *parent, bool saveMode, const DirectoryPath &directory) const
	{
		...
		extensions.push_back (make_pair (L"gst", LangString["GST_VOLUMES"]));
	...
	DEFINE_EVENT_TYPE (GST_EVENT_THREAD_EXITING);
