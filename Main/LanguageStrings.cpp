...
namespace GostCrypt
{
	...
	void LanguageStrings::Init ()
	{
		...
		Map["OPEN_NEW_VOLUME"] = _("Specify a New GostCrypt Volume");
		Map["PARAMETER_INCORRECT"] = _("Parameter incorrrect");
		Map["SELECT_KEYFILES"] = _("Select Keyfiles");
		Map["START_GST"] = _("Start GostCrypt");
		...
		Map["CONFIRM_BACKGROUND_TASK_DISABLED"] = _("WARNING: If the GostCrypt Background Task is disabled, the following functions, depending on the platform, will be disabled whenever you exit GostCrypt:\n\n1) Auto-dismount (e.g., upon logoff, time-out, etc.)\n2) Notifications (e.g., when damage to hidden volume is prevented)\n3) Tray icon\n\nNote: You may shut down the Background Task anytime by right-clicking the GostCrypt tray icon and selecting 'Exit'.\n\nAre you sure you want to disable the GostCrypt Background Task?");
		Map["CONFIRM_EXIT"] = _("WARNING: If GostCrypt exits now, the following functions, depending on the platform, will be disabled:\n\n1) Auto-dismount (e.g., upon logoff, time-out, etc.)\n2) Notifications (e.g., when damage to hidden volume is prevented)\n3) Tray icon\n\nNote: If you do not wish GostCrypt to continue running in background after you close its window, disable the Background Task in the Preferences.\n\nAre you sure you want GostCrypt to exit?");
		Map["DAMAGE_TO_HIDDEN_VOLUME_PREVENTED"] = _("WARNING: Data were attempted to be saved to the hidden volume area of the volume \"{0}\"!\n\nGostCrypt prevented these data from being saved in order to protect the hidden volume. This may have caused filesystem corruption on the outer volume and the operating system may have reported a write error (\"Delayed Write Failed\", \"The parameter is incorrect\", etc.). The entire volume (both the outer and the hidden part) will be write-protected until it is dismounted.\n\nWe strongly recommend that you restart the operating system now.");
		Map["ENTER_PASSWORD"] = _("Enter password");
		Map["ENTER_PASSWORD_FOR"] = _("Enter password for \"{0}\"");
		Map["ENTER_GST_VOL_PASSWORD"] = _("Enter GostCrypt Volume Password");
		...
		Map["CHECKING_FS"] = _("Checking the file system on the GostCrypt volume mounted as {0}...");
		Map["REPAIRING_FS"] = _("Attempting to repair the file system on the GostCrypt volume mounted as {0}...");
