...
namespace GostCrypt
{
	...
	{
		GST_UNUSED_VAR ucontext_t *context = (ucontext_t *) contextArg;
...
#	endif
		...
		wxString msg = L"A critical error has occurred and GostCrypt must be terminated. If this is caused by a bug in GostCrypt, we would like to fix it. To help us, you can send us an automatically generated error report containing the following items:\n\n- Program version\n- Operating system version\n- Hardware architecture\n- Checksum of GostCrypt executable\n- Error category\n- Error address\n";
#if wxUSE_STACKWALKER == 1
		msg += L"- GostCrypt call stack\n";
#endif
	...
	void FatalErrorHandler::OnTerminate ()
	{
		...
		catch (Exception &e)
		{
			...
			if (exName.find (L"GostCrypt::") != string::npos)
				exName = exName.Mid (11);

			wxString exPos = StringConverter::ToWide (e.what());
			if (exPos.find (L"GostCrypt::") != string::npos)
				exPos = exPos.Mid (11);
			...
			wxString msg = L"An unhandled exception has occurred and GostCrypt must be terminated. If this is caused by a bug in GostCrypt, we would like to fix it. To help us, you can send us an automatically generated error report containing the following items:\n\n- Program version\n- Operating system version\n- Hardware architecture\n- Checksum of GostCrypt executable\n- Error description\n- Error location\n";
#if wxUSE_STACKWALKER == 1
			msg += L"- GostCrypt call stack\n";
#endif
			msg += L"\nIf you select 'Yes', the following URL (which contains the entire error report) will be opened in your default Internet browser.\n\n";
