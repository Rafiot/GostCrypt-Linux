...
namespace GostCrypt
{
	...
	void UserInterface::CheckRequirementsForMountingVolume () const
	{
		...
		if (!Preferences.NonInteractive)
		{
			if (!SystemInfo::IsVersionAtLeast (2, 6, 24))
				ShowWarning (_("Your system uses an old version of the Linux kernel.\n\nDue to a bug in the Linux kernel, your system may stop responding when writing data to a GostCrypt volume. This problem can be solved by upgrading the kernel to version 2.6.24 or later."));
		}
#endif // GST_LINUX
	}
...
#ifdef GST_LINUX
			if (string (volume.VirtualDevice).find ("/dev/mapper/gostcrypt") != 0)
			{
#endif
	...
	wxString UserInterface::ExceptionToMessage (const exception &ex) const
	{
		...
		size_t pos = typeName.find ("GostCrypt::");
		if (pos != string::npos)
		{
			return StringConverter::ToWide (typeName.substr (pos + string ("GostCrypt::").size()))
...
#ifdef GST_MACOSX
		EX2MSG (HigherFuseVersionRequired,			_("GostCrypt requires MacFUSE 1.3 or later."));
#endif
		...
		case CommandId::Help:
			{
				wstring helpText = StringConverter::ToWide (
					"Synopsis:\n"
					"\n"
					"gostcrypt [OPTIONS] COMMAND\n"
					"gostcrypt [OPTIONS] VOLUME_PATH [MOUNT_DIRECTORY]\n"
					"\n"
					"\n"
					"Commands:\n"
					"\n"
					...
					" 1) Path to the encrypted GostCrypt volume.\n"
					...
					" command with option -o when a filesystem on a GostCrypt volume is mounted.\n"
					...
					" Specifies comma-separated mount options for a GostCrypt volume:\n"
					...
					"IMPORTANT:\n"
					"\n"
					"If you want to use GostCrypt, you must follow the security requirements and\n"
					"security precautions listed in chapter 'Security Requirements and Precautions'\n"
					"in the GostCrypt documentation (file 'GostCrypt User Guide.pdf').\n"
					"\n"
					"\nExamples:\n\n"
					"Create a new volume:\n"
					"gostcrypt -t -c\n"
					"\n"
					"Mount a volume:\n"
					"gostcrypt volume.gst /media/gostcrypt1\n"
					"\n"
					"Mount a volume as read-only, using keyfiles:\n"
					"gostcrypt -m ro -k keyfile1,keyfile2 volume.gst\n"
					"\n"
					"Mount a volume without mounting its filesystem:\n"
					"gostcrypt --filesystem=none volume.gst\n"
					"\n"
					"Mount a volume prompting only for its password:\n"
					"gostcrypt -t -k \"\" --protect-hidden=no volume.gst /media/gostcrypt1\n"
					"\n"
					"Dismount a volume:\n"
					"gostcrypt -d volume.gst\n"
					"\n"
				...
				{
					wxDialog dialog (nullptr, wxID_ANY, _("GostCrypt Command Line Help"), wxDefaultPosition);
