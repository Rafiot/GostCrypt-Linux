...
namespace GostCrypt
{
	...
	void TextUserInterface::CreateVolume (shared_ptr <VolumeCreationOptions> options) const
	{
		...
		if (options->Path.IsDevice())
			...
			options->SectorSize = GST_SECTOR_SIZE_FILE_HOSTED_VOLUME;
		...
		if (options->Type == VolumeType::Hidden)
		{
			...
			if (hostSize < GST_MIN_HIDDEN_VOLUME_HOST_SIZE)
				throw_err (StringFormatter (_("Minimum outer volume size is {0}."), SizeToString (GST_MIN_HIDDEN_VOLUME_HOST_SIZE)));
		}

		uint64 minVolumeSize = options->Type == VolumeType::Hidden ? GST_MIN_HIDDEN_VOLUME_SIZE : GST_MIN_VOLUME_SIZE;
		uint64 maxVolumeSize = options->Type == VolumeType::Hidden ? VolumeLayoutV2Normal().GetMaxDataSize (hostSize) - GST_MIN_FAT_FS_SIZE : GST_MAX_VOLUME_SIZE_GENERAL;
		...
		if (options->Filesystem == VolumeCreationOptions::FilesystemType::FAT
			&& (filesystemSize < GST_MIN_FAT_FS_SIZE || filesystemSize > GST_MAX_FAT_SECTOR_COUNT * options->SectorSize))
		{
